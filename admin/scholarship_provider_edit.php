<?php
	...
	if(isset($_POST['edit'])){
		...
		if (empty($id) ||empty($username) || empty($firstname) || empty($lastname) || empty($scholarship_program) || empty($password)){
		...
		&& updaterecord('staff',['username','sp_id'],[$username,$scholarship_program]);
		if($result){
			$_SESSION['success'] = "Scholarship Provider Updated.";
		}else{
			$_SESSION['error'] = "Something went wrong updating scholarship provider.";
		}
                    ...
                    ?>
                        <form role="form" method="post">
                            ...
                            <div class="row">
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label class="form-control-label" for="role">Scholarship Program</label>
                                        <select class="form-control" name="scholarship_program" title="Scholarship Program" id="scholarship_program">
											<option value='<?php echo $row['sp_id'];?>' selected><?php echo $row['sp_name'];?></option>
											<?php
											$options=getall('scholarship_program');
